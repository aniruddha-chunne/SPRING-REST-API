...
import in.bhushansirgur.springrestapi.model.Department;
import in.bhushansirgur.springrestapi.model.Empclass;
import in.bhushansirgur.springrestapi.model.Employee;
import in.bhushansirgur.springrestapi.repository.DepartmentRepository;
import in.bhushansirgur.springrestapi.repository.EmpclassRepository;
...
@Controller
@RestController
public class EmpclassController 
{
...
//	}
//	
	@PostMapping("/empclass")
	public ResponseEntity<Empclass> saveEmployeesInDatabase( @Valid  @RequestBody EmpclassRequest eRequest)
	{
		...
		Empclass eclass = new Empclass(dept);
		
		eclass.setDepartment(dept);
		eclass.setId(24L);
		
		empClassRepository.save(eclass);
		
		// Http Status 201 - HttpStatus.CREATED
//		return "saving the employee details to the database : " + emp.getAge();
		
		return  new ResponseEntity<Empclass>(eclass, HttpStatus.CREATED);
